...
window.lials.main = (function () {
    ...
    var usersUsername = "",
        ...
        renderError = function (error) {
            var errorDeleteButton, errors = document.getElementById("errors"),
        ...
        loopThroughData = function (data, toRun, genericMessage) {
            ...
            //otherwise check feedback and show user
            else {
        ...
        renderCompletion = function (goal) {
            ...
            //checks if goal has been posted as completed or not
        ...
        renderPrivate = function (user) {
            ...
            //checks if the users private attribute is as private or not
            if (user.Private == 1) {
        ...
        follow = function (user, img) {
            //checks if users not following or already following
            if (img.src.includes("not")) {
                window.lials.xhr.load({
                    "method": "POST",
                    "url": "follows",
                    "query": "me=" + usersUsername + "&user=" + user,
                ...
                window.lials.xhr.load({
                    "method": "DELETE",
                    "url": "follows?me=" + usersUsername + "&user=" + user,
                    "load": renderUnFollowUpdate
                });
            }
        },
        //set to eaither like a goal or unlike a goal
        like = function (goalID, img) {
            //checks if goal is liked already
            if (img.src.includes("not")) {
                window.lials.xhr.load({
                    "method": "POST",
                    "url": "likes",
                    "query": "me=" + usersUsername + "&goalID=" + goalID,
                    "load": gotLikeUpdate
                });
            }
            else {
                window.lials.xhr.load({
                    "method": "DELETE",
                    "url": "likes?me=" + usersUsername + "&goalID=" + goalID,
       ...
       //create a edit goal form
        ...
        gotGoals = function (result) {
            ...
            //if data doesn't exist and theres no feedback assume theres no error and no goals to show
            if (dataExists == false) {
        ...
        postGoal = function () {
            ...
            if (goal.value.trim() == "" && due.value.trim() == "") {
                ...
                document.getElementById("goalFormFeedback").innerHTML = "Input fields needs to be filled.";
            } else {
                //checks if goal is empty
                if (goal.value.trim() == "") {
                    due.style.borderColor = "initial";
                    goal.style.borderColor = "red";
                    document.getElementById("goalFormFeedback").innerHTML = "Goal field needs to be filled.";
                }
                //checks if due date is empty
                else if (due.value.trim() == "") {
                    due.style.borderColor = "red";
                    goal.style.borderColor = "initial";
                    document.getElementById("goalFormFeedback").innerHTML = "Due date field needs to be filled.";
                }
        ...
        setUpUser = function (user) {
            ...
            //add a listener for when user does a input in search bar
            search.addEventListener("input", doSearch);
            //add a listener for when user clicks to post a goal
            ...
            document.getElementById("logo").addEventListener("click", getHomeGoals);
            document.getElementById("mobileLogo").addEventListener("click", getHomeGoals);
        ...
        signedUp = function (result) {
            ...
            //if data returned isn't valid check for feedback and print it out or show generic error message
            if (dataExists == false) {
                if (result.meta.feedback) {
        ...
        loggedIn = function (result) {
            ...
            var dataExists = loopThroughData(result, setUpUser);
            //if data wasn't valid check for feedback and print it out or show generic error message
            if (dataExists == false) {
                if (result.meta.feedback) {
                    document.getElementById("userFormFeedback").innerHTML = result.meta.feedback;
                } else {
        ...
        logIn = function () {
            ...
            if (username.value.trim() == "" && password.value.trim() == "") {
                ...
                document.getElementById("userFormFeedback").innerHTML = "Input fields needs to be filled.";
            } else {
                if (username.value.trim() == "") {
                    password.style.borderColor = "initial";
                    username.style.borderColor = "red";
                    document.getElementById("userFormFeedback").innerHTML = "Username field needs to be filled.";
                }
                else if (password.value.trim() == "") {
                    password.style.borderColor = "red";
                    username.style.borderColor = "initial";
                    document.getElementById("userFormFeedback").innerHTML = "Password field needs to be filled.";
                }
                //else inputs are filled
                else {
                    username.style.borderColor = "initial";
